...
import { Play, Square, Volume2, Timer as TimerIcon, Bookmark, Trash2, Share2, Check } from 'lucide-react';
import { Dialog, DialogOverlay, DialogContent, DialogHeader, DialogTitle, DialogClose, DialogBody } from '@/components/Dialog';
    ...
    if (prevVolumeRef.current !== masterVolume && volumeRef.current) {
      ...
      const timer = setTimeout(() => {
        if (volumeRef.current) {
          volumeRef.current.classList.remove('scale-120', 'text-white');
        ...
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 flex items-center justify-between">
          ...
          <div className="relative flex items-center gap-4 sm:gap-6">
            <button
              ...
              aria-label={timeRemaining !== null ? `Edit timer, ${formatTime(timeRemaining)} remaining` : 'Set sleep timer'}
            ...
            <button
              type="button"
              onClick={() => {
                trackClick('playback', isPlaying ? 'master_pause' : 'master_play');
                toggleMasterPlay();
              }}
          ...
          <DialogContent>
            ...
            <DialogBody>
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                {[15, 30, 45, 60, 90, 120].map(mins => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      trackClick('timer', `timer_${mins}m`);
                      startTimer(mins);
                    }}
              ...
              <div className="flex items-center gap-3">
                <label htmlFor="customTimerMinutes" className="sr-only">Custom minutes</label>
                <input 
                ...
                <button 
                  ...
                  onClick={() => {
                    ...
                    startTimer(parseInt(customMinutes) || 30);
                  }}
                    ...
                    <div key={mix.id} className="flex items-center justify-between p-3 border border-[#333] bg-[#111] hover:border-[#555] transition-colors group">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          type="button"
                          onClick={() => {
                            trackClick('presets', `load_preset_${mix.name}`);
                            loadMix(mix.mixStr);
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            trackClick('presets', `share_preset_${mix.id}`);
                            handleShare(mix.mixStr, mix.id);
                          }}
                          aria-label={copiedId === mix.id ? 'Link copied' : `Share ${mix.name}`}
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            trackClick('presets', `delete_preset_${mix.id}`);
                            deleteMix(mix.id);
                          }}
