...
import { Dialog, DialogOverlay, DialogContent, DialogHeader, DialogTitle, DialogClose, DialogBody } from './Dialog';
...
export function MixerPanel() {
  ...
  const activeSoundIds = useMemo(() => Object.keys(activeSounds), [activeSounds]);
  const activeCount = activeSoundIds.length;
  const displayIds = activeSoundIds.slice(0, 4);
  const remainingCount = activeCount - 4;

  const handleOpenSave = useCallback(() => {
    const names = displayIds.map(id => SOUNDS_BY_ID[id]?.name).filter(Boolean);
      ...
      <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 flex items-center justify-between h-full">
        ...
        <div className="flex items-center gap-2 sm:gap-4 ml-4 sm:ml-6 flex-shrink-0">
          ...
          <button
            ...
            aria-label={isSaved ? 'Preset saved' : 'Save preset'}
