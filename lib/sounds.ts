...
const getIdFromPath = (src: string) =>
    src.replace(/^\/sounds\//, '').replace(/\.[a-z0-9]+$/i, '');
...
export const formatCategoryLabel = (category: SoundCategory) =>
...
const categoryAccumulator: Record<SoundCategory, SoundDef[]> =
...
export const getSoundById = (id: string): SoundDef | undefined =>
    SOUNDS_BY_ID[id];
export const getSoundsByCategory = (category: SoundCategory): SoundDef[] =>
    SOUNDS_BY_CATEGORY[category] || [];
